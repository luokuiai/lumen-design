...
const getInitialLocale = () => {
  ...
  try {
    return window.localStorage.getItem(localeStorageKey) === "en-US"
      ? enUS
      : zhCN;
  } catch {
    return zhCN;
...
const initialTheme: Theme = (() => {
  ...
  try {
    ...
    return storedTheme === "paper" || storedTheme === "prism"
      ? storedTheme
      : "clarity";
  } catch {
    return "clarity";
...
const initialAccent: Accent = (() => {
  ...
  try {
    return window.localStorage.getItem(accentStorageKey) === "purple"
      ? "purple"
      : "blue";
  } catch {
    return "blue";
...
const renderSections: Section[] = [
  {
    id: "typography",
    title: "Typography",
    description: "标题、正文和辅助文字层级。",
    keywords: "Typography H1 H2 H3 H4 H5 H6 Body Caption",
    icon: TypeIcon,
  },
  {
    id: "buttons",
    title: "Buttons",
    description: "按钮、徽标、Chip、头像和 Tooltip。",
    keywords:
      "Button useLongPress ContextMenu DragHandle Badge Chip Avatar Tooltip",
    icon: Plus,
  },
  {
    id: "forms",
    title: "Forms",
    description: "输入、校验、开关、单选和多行文本。",
    keywords:
      "Input NumberInput OtpInput FormField Textarea Checkbox Radio RadioGroup Rating Switch Slider",
    icon: Check,
  },
  {
    id: "pickers",
    title: "Pickers",
    description:
      "选择器、输入建议、级联选择、树选择、穿梭框、日历、日期和时间选择。",
    keywords:
      "Select Combobox Cascader TreeSelect Transfer Calendar DatePicker TimePicker DateTimePicker",
    icon: CalendarDays,
  },
  {
    id: "data",
    title: "Data Display",
    description: "文件类型、数据表格、列表、滚动区域、分隔和折叠内容。",
    keywords:
      "FileTypeIcon DataTable List ListItem VirtualList SwipeActions Pagination Scrollbar Divider Collapse Accordion",
    icon: Table2,
  },
  {
    id: "navigation",
    title: "Navigation",
    description: "应用栏、工具栏、底部导航和页面导航。",
    keywords:
      "AppBar Toolbar BottomNavigation Breadcrumb Carousel Tabs Steps DropdownMenu Timeline SideNav",
    icon: MoreHorizontal,
  },
  {
    id: "overlays",
    title: "Overlays",
    description: "对话框、抽屉、命令面板、确认和消息提示。",
    keywords: "Dialog Drawer CommandPalette ConfirmDialog Toast",
    icon: Bell,
  },
  {
    id: "feedback",
    title: "Feedback",
    description: "页面提示、加载、进度、空状态、上传和骨架屏。",
    keywords:
      "Alert Spinner Progress Empty FileUpload Skeleton SegmentedControl",
    icon: Settings,
  },
];

const toDemoId = (title: string) =>
  title
...
const galleryCategories: GalleryCategory[] = [
  {
    ...
    demos: [
      ...
      demo(
        "Locale",
        "typography",
        "LumenProvider, Pagination, Select, enUS",
  ...
  },
  {
    ...
    keywords:
      "Button Fab Toolbar DropdownMenu ContextMenu useLongPress DragHandle SwipeActions",
    icon: Plus,
    demos: [
      demo(
        "Button",
      ...
      ),
      {
        ...demo(
          ...
          "buttons",
          "Fab",
          '    <Fab position="static" icon={<Plus size={18} />} aria-label="新建任务" />',
          "Plus",
          undefined,
          ["Icon only", "Extended", "Expandable", "Submenu"],
        ),
        codeByCardTitle: {
          "Icon only": usageExample(
            "Fab",
      ...
      demo(
        ...
        undefined,
        "const [action, setAction] = useState('暂无');",
        ["Context menu"],
      ),
      demo(
        "useLongPress",
        "buttons",
        "Button, Typography, useLongPress",
  ...
  },
  {
    ...
    keywords:
      "Form Validation FormField Input Textarea NumberInput OtpInput Checkbox Radio Switch SegmentedControl Slider Rating Select Combobox Cascader TreeSelect Transfer Calendar Date Time FileUpload",
    icon: Check,
    demos: [
      demo(
        "Form / FormField",
        "forms",
        "Form, FormField, Input, Button, Alert, SegmentedControl, Toast",
        '    <Form values={values} validate={validate} onFinish={submit}>\n      <FormField name="name" label="姓名" required>\n        {(props) => <Input {...props} value={values.name} onChange={(event) => setValues({ name: event.target.value })} />}\n      </FormField>\n      <Button type="submit">校验并提交</Button>\n    </Form>',
        undefined,
      ...
      demo(
        ...
        "forms",
        "Input",
        '    <Input aria-label="项目名称" placeholder="请输入项目名称" />',
      ),
      demo(
        "Textarea",
        "forms",
      ...
      demo(
        ...
        "feedback",
        "SegmentedControl",
        '    <SegmentedControl value="all" options={options} onChange={setValue} />',
      ),
      demo(
        "Slider",
        "forms",
        "Slider",
        "    <Slider value={60} onChange={() => undefined} />",
      ),
      demo(
        "Rating",
        "forms",
        "Rating",
        "    <Rating value={4} onChange={() => undefined} />",
      ),
      demo(
        "Select",
        "pickers",
      ...
      demo(
        ...
        "pickers",
        "Cascader",
        "    <Cascader options={options} value={[]} onChange={() => undefined} />",
      ),
      demo(
        "TreeSelect",
        "pickers",
        "TreeSelect",
        '    <TreeSelect nodes={nodes} value="frontend" onChange={setValue} searchable />',
      ),
      demo(
        "Transfer",
        "pickers",
      ...
      demo(
        ...
        "pickers",
        "Calendar",
        '    <Calendar value="2026-09-04" onChange={() => undefined} />',
      ),
      demo(
        "DatePicker",
        "pickers",
        "DatePicker",
        '    <DatePicker value="2026-09-04" onChange={setValue} />',
      ),
      demo(
        "TimePicker",
        "pickers",
        "TimePicker",
        '    <TimePicker value="09:30" onChange={setValue} />',
      ),
      demo(
        "DateTimePicker",
        "pickers",
        "DateTimePicker",
        '    <DateTimePicker label="开始时间" value="2026-09-04 09:30:00" onChange={setValue} />',
      ),
      demo(
        "FileList",
        "feedback",
        "FileList",
        "<FileList items={[{ id: 'report', name: 'report.pdf', progress: 58 }]} />",
        undefined,
        undefined,
        ["FileList", "FileList Wrapped"],
      ),
      demo(
        "FileUpload",
        "feedback",
        ...
        undefined,
        undefined,
        ["FileUpload", "FileUpload Progress", "FileUpload Compact"],
      ),
    ],
  },
  {
    ...
    keywords:
      "AppBar BottomNavigation SideNav Breadcrumb Tabs Steps Pagination ScrollToEdge",
    icon: MoreHorizontal,
    demos: [
      demo(
        "SideNav",
        "navigation",
        "SideNav",
        '    <SideNav sections={[{ items: [{ value: "overview", label: "总览" }, { value: "settings", label: "设置" }] }]} />',
      ),
      demo(
        "AppBar",
        "navigation",
        "AppBar, Button, Typography",
      ...
      demo(
        ...
        "Bell, CalendarDays, Star, UserRound",
      ...
      demo(
        ...
        "navigation",
        "Steps",
        "    <Steps current={1} items={[{ title: '提交' }, { title: '完成' }]} />",
      ),
      demo(
        "Pagination",
        "data",
      ...
      demo(
        ...
        "navigation",
        "ScrollToEdge",
        '    <ScrollToEdge direction="top" threshold={240} />',
      ),
    ],
  },
  {
    ...
    keywords:
      "Avatar Badge Chip FileTypeIcon List VirtualList Scrollbar DataTable Tree Timeline Carousel Collapse Accordion Divider Watermark",
    icon: Table2,
    demos: [
      ...
      demo(
        ...
        "buttons",
        "Chip",
        '    <Chip tone="neutral">设计系统</Chip>',
      ),
      demo(
        "FileTypeIcon",
        "data",
        "FileTypeIcon",
        '    <FileTypeIcon fileName="proposal.pdf" />',
      ),
      demo(
        "List",
        "data",
      ...
      demo(
        ...
        "data",
        "Scrollbar",
        '    <Scrollbar className="h-64">{content}</Scrollbar>',
      ),
      demo(
        "DataTable",
        "data",
        ...
        undefined,
        undefined,
        ["DataTable · Sticky Header", "DataTable · Embedded"],
      ),
      demo(
        "Tree",
        "data",
      ...
      demo(
        ...
        "data",
        "Watermark",
        "    <Watermark content={['Lumen Design', '内部资料 · 用户 1024']}>\n      <div className=\"p-6\">敏感业务内容</div>\n    </Watermark>",
      ),
    ],
  },
  {
    ...
    demos: [
      ...
      demo(
        ...
        "overlays",
        "Button, Toast",
        "    <Button onClick={() => Toast.success('组件配置已成功保存，新的设置将在下次打开页面时自动生效。')}>Toast</Button>",
      ),
  ...
  },
  {
    ...
    keywords:
      "Dialog Drawer BottomSheet ConfirmDialog CommandPalette Popover Tooltip",
    icon: Bell,
    demos: [
      demo(
        "Popover",
        "overlays",
        "Button, Popover",
        "    <Popover trigger={<Button>查看详情</Button>}>\n      上下文内容\n    </Popover>",
      ),
      demo(
        "Dialog",
        "overlays",
        "Button, Dialog",
        '    <Dialog open={open} title="编辑配置" description="修改当前配置项。" onRequestClose={() => setOpen(false)}\n      footer={<Button onClick={() => setOpen(false)}>保存</Button>}\n    >{content}</Dialog>',
      ),
      demo(
        "ConfirmDialog",
        "overlays",
        "Button, ConfirmDialog",
        '    <Button variant="destructive" onClick={() => setOpen(true)}>打开 Confirm</Button>',
      ),
      demo(
        "Drawer",
        "overlays",
        "Button, Drawer",
        '    <Drawer open={open} closeOnSwipe title="配置面板" description="调整页面配置。" onRequestClose={() => setOpen(false)}>{content}</Drawer>',
      ),
      demo(
        "BottomSheet",
        "overlays",
      ...
      demo(
        "CommandPalette",
        "overlays",
        "Button, CommandPalette",
        "    <Button onClick={() => setOpen(true)}>打开 CommandPalette</Button>",
      ),
    ],
  },
];
...
const playgroundMessages = {
  ...
  "en-US": {
    locale: "en-US",
    appDescription:
      "A complete preview and interaction-checking workspace for the component library.",
    ...
    categories: {
      foundations: {
        title: "Foundations",
        description: "Typography and foundational content styles.",
      },
      actions: {
        title: "Actions",
        description: "Triggers, tool groups, and primary page actions.",
      },
      forms: {
        title: "Forms",
        description:
          "Inputs, selections, date and time controls, and file submission.",
      },
      navigation: {
        title: "Navigation",
        description: "Application-level and page-level navigation structures.",
      },
      "data-display": {
        title: "Data Display",
        description: "Status, lists, tables, and structured content.",
      },
      feedback: {
        title: "Feedback",
        description: "Results, progress, loading, and empty states.",
      },
      overlays: {
        title: "Overlays",
        description: "Dialogs, drawers, and contextual actions above the page.",
      },
    },
  },
} as const;
...
type PlaygroundMessages =
  (typeof playgroundMessages)[keyof typeof playgroundMessages];
const PlaygroundMessagesContext = createContext<PlaygroundMessages>(
  playgroundMessages["zh-CN"],
);
...
const componentGuides: Record<string, ComponentGuide> = {
  fab: {
    summary:
      "浮动操作按钮用于突出当前页面最重要、最常用的单一操作，适合创建、编辑或快速添加。",
    usage: [
    ...
    props: [
      ...
      },
      {
        ...
        type: "string",
        defaultValue: "-",
        description: "设置任意 CSS 背景色，并覆盖 variant 的背景。",
      },
      {
        name: "foregroundColor",
        type: "string",
        defaultValue: "on-primary",
        description: "设置文字与图标颜色。",
      },
      {
        name: "label",
        type: "ReactNode",
        defaultValue: "-",
        description: "扩展状态显示的文字，也可作为折叠状态的可访问名称。",
      },
      {
        name: "extended",
        type: "boolean",
        defaultValue: "自动",
        description: "显式控制带文字或仅图标形态。未设置时根据 label 判断。",
      },
      {
        name: "size",
        type: "'sm' | 'md' | 'lg'",
        defaultValue: "'sm'",
        description: "控制 36、44、52px 三档高度。",
      },
      {
        name: "variant",
        type: "ButtonVariant",
        defaultValue: "'primary'",
        description: "设置主色、次级、描边或危险操作样式。",
      },
      {
        name: "position",
        type: "'fixed' | 'absolute' | 'static'",
        defaultValue: "'fixed'",
        description: "决定相对视口、容器或普通文档流定位。",
      },
      {
        name: "placement",
        type: "'bottom-end' | 'bottom-start' | 'top-end' | 'top-start'",
        defaultValue: "'bottom-end'",
        description: "设置悬浮位置，并自动适配 RTL。",
      },
      {
        name: "offset",
        type: "number | string",
        defaultValue: "16",
        description: "设置距定位边缘的间距。",
      },
      {
        name: "safeArea",
        type: "boolean",
        defaultValue: "true",
        description: "在顶部或底部叠加设备安全区。",
      },
      {
        name: "active",
        type: "boolean",
      ...
      },
      {
        ...
        type: "boolean",
        defaultValue: "false",
        description: "显示加载图标并禁止重复点击。",
      },
      {
        name: "actions",
        type: "readonly FabAction[]",
        defaultValue: "[]",
        description: "配置点击主按钮后展开的子 Fab。",
      },
      {
        name: "open",
        type: "boolean",
        defaultValue: "-",
        description: "受控设置子菜单是否展开。",
      },
      {
        name: "defaultOpen",
        type: "boolean",
        defaultValue: "false",
        description: "设置非受控子菜单初始状态。",
      },
      {
        name: "onOpenChange",
        type: "(open: boolean) => void",
        defaultValue: "-",
        description: "子菜单展开状态变化时触发。",
      },
      {
        name: "menuDirection",
        type: "'up' | 'down' | 'start' | 'end'",
        defaultValue: "自动",
        description: "设置子 Fab 展开方向；默认根据 placement 向页面内部展开。",
      },
    ],
  },
  toolbar: {
    ...
    usage: [
      "使用方向键、Home 和 End 在工具间移动焦点。",
      "操作较多时开启 wrap，紧凑场景使用 sm。",
    ],
    props: [
      {
        name: "size",
        type: "'sm' | 'md' | 'lg'",
        defaultValue: "'md'",
        description: "设置工具栏最小高度与水平留白。",
      },
      {
        name: "wrap",
        type: "boolean",
        defaultValue: "false",
        description: "空间不足时允许工具换行。",
      },
      {
        name: "ariaLabel",
        type: "string",
        defaultValue: "'工具栏'",
        description: "描述这一组工具的用途。",
      },
      {
        name: "children",
        type: "ReactNode",
        defaultValue: "-",
        description: "按钮、输入框、分隔符或其他工具。",
      },
    ],
  },
  "app-bar": {
    ...
    usage: [
      "移动页面通常使用居中标题；信息密集页面可使用 start 对齐。",
      "fixed 和 sticky 会参与视口布局，局部演示使用 absolute。",
    ],
    props: [
      {
        name: "title",
        type: "ReactNode",
        defaultValue: "-",
        description: "必填，当前页面标题。",
      },
      {
        name: "leading",
        type: "ReactNode",
        defaultValue: "-",
        description: "标题前的返回或导航操作。",
      },
      {
        name: "actions",
        type: "ReactNode",
        defaultValue: "-",
        description: "标题后的页面级操作。",
      },
      {
        name: "position",
        type: "'fixed' | 'absolute' | 'sticky' | 'static'",
        defaultValue: "'fixed'",
        description: "设置应用栏定位方式。",
      },
      {
        name: "titleAlign",
        type: "'start' | 'center'",
        defaultValue: "'center'",
        description: "设置标题对齐方式。",
      },
      {
        name: "size",
        type: "'sm' | 'md' | 'lg'",
  ...
  "bottom-navigation": {
    summary:
      "底部导航用于移动端的应用级主目的地切换，适合 3 到 5 个稳定且同级的入口。",
    usage: [
      "保持入口顺序稳定，不把临时操作放入底部导航。",
      "受控 value 决定当前项，onChange 负责同步路由或页面状态。",
    ],
    props: [
      {
        name: "items",
        ...
        defaultValue: "-",
        description: "必填，配置 value、label、icon、badge、href 和 disabled。",
      },
      {
        name: "value",
        type: "string",
        defaultValue: "-",
        description: "当前选中项的 value。",
      },
      {
        name: "onChange",
        type: "(value, item) => void",
        defaultValue: "-",
        description: "选择可用项目时触发。",
      },
      {
        name: "position",
        type: "'fixed' | 'absolute' | 'static'",
        defaultValue: "'fixed'",
        description: "设置相对视口、局部容器或文档流定位。",
      },
      {
        name: "safeArea",
        type: "boolean",
        defaultValue: "true",
        description: "为底部手势区域增加安全间距。",
      },
      {
        name: "active",
        type: "boolean",
        defaultValue: "true",
        description: "控制导航是否显示。",
      },
      {
        name: "ariaLabel",
        type: "string",
        defaultValue: "'底部导航'",
        description: "设置导航区域的可访问名称。",
      },
    ],
  },
};
...
const enGuideSummaries: Record<string, string> = {
  ...
  toolbar:
    "Toolbars organize actions, filters, and view controls that directly relate to the current content.",
  "app-bar":
    "App bars establish page context with navigation, a title, and a small set of page-level actions.",
  "bottom-navigation":
    "Bottom navigation switches between three to five stable, top-level destinations in a mobile application.",
};
...
const getRouteFromHash = () => {
  ...
  if (typeof window === "undefined") {
    return {
      ...
      demoId: fallbackCategory.demos[0]!.id,
    };
  }

  const [rawCategoryId, rawDemoId] = window.location.hash.slice(1).split("/");
  const categoryId =
    legacyCategoryAliases[rawCategoryId ?? ""] ?? rawCategoryId;
  const demoId = legacyDemoAliases[rawDemoId ?? ""] ?? rawDemoId;
  const requestedCategory = galleryCategories.find(
    (item) => item.id === categoryId,
  );
  const category = requestedCategory?.demos.some((item) => item.id === demoId)
    ? requestedCategory
    : (galleryCategories.find((item) =>
        item.demos.some((entry) => entry.id === demoId),
      ) ??
      requestedCategory ??
      fallbackCategory);
  const selectedDemo =
    category.demos.find((item) => item.id === demoId) ?? category.demos[0]!;
...
const safetyEventColumns: DataTableColumn<SafetyEvent>[] = [
  {
    ...
    minWidth: 112,
    render: (event) => (
      <span className="font-normal text-[var(--lumen-color-text)]">
        {event.id}
      </span>
    ),
  },
  ...
  return (
    ...
    >
      <DragHandle
        ...
        active={isDragging}
        label={
          locale === "en-US" ? `Drag ${label} to reorder` : `拖动${label}排序`
        }
      />
      <span className="text-[14px] text-[var(--lumen-color-text)]">
        {label}
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={{ duration: 200, easing: "ease" }}>
        {activeItem ? (
          <DragHandleOverlay item={activeItem} locale={messages.locale} />
        ) : null}
      </DragOverlay>
    </DndContext>
  );
...
function GallerySection({
  ...
  const messages = useContext(PlaygroundMessagesContext);
  const [generatedComponentApi, setGeneratedComponentApi] = useState<
    Record<string, GeneratedPropDoc[]>
  >({});
  const guide = activeDemo ? componentGuides[activeDemo.demo.id] : undefined;
  const guideUsage =
    guide && messages.locale === "en-US"
      ? (enGuideUsage[activeDemo!.demo.id] ?? guide.usage)
      : guide?.usage;
  const localizedChildren =
    messages.locale === "en-US" ? localizeDemoNode(children) : children;
  const apiSections =
    activeDemo?.demo.apiComponents.flatMap((componentName) => {
      ...
      return [
        {
          ...
          props: [
            ...mergedProps,
            ...guide.props.filter((prop) => !generatedNames.has(prop.name)),
          ],
        },
      ];
    }) ?? [];
  ...
  return (
    <section id={section.id} className="gallery-section">
      ...
      {apiSections.length ? (
        <section
          className="component-api"
          aria-labelledby={`${activeDemo!.demo.id}-api-title`}
        >
          <div className="component-api-card">
            <div className="component-api-heading">
              <span>API</span>
              <h3 id={`${activeDemo!.demo.id}-api-title`}>
                {messages.properties}
              </h3>
            </div>
            {apiSections.map(({ componentName, props }) => (
              <div key={componentName} className="component-api-group">
                <h4>{componentName}</h4>
                <div
                  className="component-api-table-wrap lumen-scrollbar"
                  data-size="sm"
                >
                      ...
                      {props.map((prop) => (
                        <tr key={prop.name}>
                          <td>
                            <code>{prop.name}</code>
                            {prop.required ? (
                              <span className="component-api-required">
                                {messages.required}
                              </span>
                            ) : null}
                          </td>
...
const tsxTokenPattern =
  /(\/\*.*?\*\/|\/\/.*|`(?:\\.|[^`\\])*`|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|<\/?[A-Za-z][\w.:-]*|\b(?:as|async|await|break|case|catch|class|const|continue|default|else|export|extends|false|finally|for|from|function|if|import|in|interface|let|new|null|of|return|switch|throw|true|try|type|undefined|while)\b|\b\d+(?:\.\d+)?\b|[A-Za-z_$][\w$-]*(?=\s*=)|[{}[\]()]|\/?>(?!=))/g;
...
function getTsxTokenClass(token: string) {
  ...
  if (
    /^(?:as|async|await|break|case|catch|class|const|continue|default|else|export|extends|false|finally|for|from|function|if|import|in|interface|let|new|null|of|return|switch|throw|true|try|type|undefined|while)$/.test(
      token,
    )
  ) {
        ...
        {code.split("\n").map((line, lineIndex) => {
          ...
          for (const match of line.matchAll(tsxTokenPattern)) {
            ...
            tokens.push(
              <span
                key={`${index}-${match[0]}`}
                className={getTsxTokenClass(match[0])}
              >
                {match[0]}
              </span>,
            );
          ...
          return (
            <span key={lineIndex} className="usage-code-line">
              <span className="usage-code-line-number" aria-hidden="true">
                {lineIndex + 1}
              </span>
              <span className="usage-code-line-content">
...
function DemoCard({
  ...
  const code =
    demoCardCodeByTitle[title] ??
    activeDemo?.demo.codeByCardTitle?.[title] ??
    activeDemo?.demo.code ??
    "";
  const localizedCode =
    messages.locale === "en-US" ? translateDemoText(code) : code;
  const localizedTitle =
    messages.locale === "zh-CN"
      ? (zhExampleNames[title] ?? zhDemoNames[title] ?? title)
      : title;
  const codePanelId = activeDemo
    ? `demo-code-${activeDemo.demo.id}-${toDemoId(title)}`
    : undefined;
  ...
  return (
    <Card className={`${wide ? "demo-card-wide " : ""}demo-card`.trim()}>
      <CardHeader className="demo-card-header">
        ...
        {activeDemo ? (
          <Tooltip
            content={codeExpanded ? messages.hideCode : messages.viewCode}
            placement="left"
          >
            <Button
              iconOnly
      ...
      <CardContent
        className={`demo-preview-surface${flush ? " demo-preview-surface-flush" : ""}`}
      >
        {children}
      </CardContent>
      {activeDemo ? (
          ...
          <div className="demo-code-collapse-inner">
            <div className="demo-code-panel">
              <div className="demo-code-header">
                <span>
                  <Code2 aria-hidden="true" size={14} /> TSX
                </span>
                <Tooltip
                  content={copied ? messages.copied : messages.copyCode}
                  placement="left"
                >
                  <Button
                    iconOnly
                    ...
                    variant="ghost"
                    aria-label={
                      copied ? messages.codeCopied : messages.copyCode
                    }
...
export default function App() {
  ...
  const [activeSection, setActiveSection] = useState(
    initialGalleryRoute.categoryId,
  );
  ...
  const [formErrorDisplay, setFormErrorDisplay] = useState<"inline" | "toast">(
    "inline",
  );
  ...
  const [swipeActionMessage, setSwipeActionMessage] =
    useState("向左右滑动事件行");
  ...
  const [stepsDirection, setStepsDirection] =
    useState<StepsDirection>("horizontal");
  ...
  const [customComboboxValue, setCustomComboboxValue] = useState<string | null>(
    null,
  );
  ...
  const [files, setFiles] = useState<File[]>(() => [
    new File(
      ["# 项目设计说明\n\n用于展示文件列表的示例附件。"],
      "应用于低压配电物联网的断路器管理方法及系统_20231024032440.md",
      { type: "text/markdown" },
    ),
    new File(
      [
        '<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40"><circle cx="20" cy="20" r="16" fill="royalblue" /></svg>',
      ],
      "项目图标.svg",
      { type: "image/svg+xml" },
    ),
  ...
  const [compactFiles, setCompactFiles] = useState<File[]>(() => [
    new File(
      ["# 设计评审记录\n\n用于展示紧凑文件列表的示例附件。"],
      "支持自动换行并完整展示文件名称的项目设计评审记录.md",
      { type: "text/markdown" },
    ),
  ]);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [commandPaletteOpen, setCommandPaletteOpen] = useState(false);
  const [dialogSelectValue, setDialogSelectValue] = useState<string | null>(
    null,
  );
  ...
  const activeCategory =
    localizedCategories.find((category) => category.id === activeSection) ??
    localizedCategories[0]!;
  const activeDemo =
    activeCategory.demos.find((item) => item.id === activeDemoId) ??
    activeCategory.demos[0]!;
  const activeSections = useMemo(() => {
    const sourceSection = renderSections.find(
      (section) => section.id === activeDemo.sourceSection,
    );
      ...
      ? [
          {
            ...
            description:
              language === "zh-CN"
                ? (componentGuides[activeDemo.id]?.summary ??
                  `${activeCategory.title} · ${activeCategory.description}`)
                : (enGuideSummaries[activeDemo.id] ??
                  `${activeCategory.title} · ${activeCategory.description}`),
          },
        ]
      : [];
  ...
  const selectNavigationItem = (demoId: string) => {
    const category = galleryCategories.find((item) =>
      item.demos.some((entry) => entry.id === demoId),
    );
          ...
          >
            <div className="mobile-nav-header">
              <GalleryBrand
                ...
                subtitle={messages.brandSubtitle}
              />
              <Button
                iconOnly
                variant="ghost"
            ...
            <Scrollbar
              ref={(element) => {
                if (element)
                  element.scrollTop = mobileNavigationScrollTopRef.current;
              }}
              ...
              onScroll={(event) => {
                mobileNavigationScrollTopRef.current =
                      ...
                      <div className="mobile-more-content">
                        ...
                        {mobileMoreView === "language" ? (
                          ...
                          <div className="p-2">
                            ...
                            ).map(([themeValue, accentValue, label]) => {
                              const selected =
                                theme === themeValue &&
                                (themeValue !== "clarity" ||
                                  accent === accentValue);
                          ...
                          <div className="p-2">
                            ...
                            >
                              ...
                              <span className="text-[12px] text-[var(--lumen-color-text-muted)]">
                                {theme === "clarity"
                                  ? `Clarity ${accent === "blue" ? "Blue" : "Purple"}`
                                  : theme === "paper"
                                    ? "Paper"
                                    : "Prism"}
                              </span>
                  ...
                  <div className="desktop-header-actions">
                    <DropdownMenu
                      ...
                      trigger={({ open, menuId, toggle }) => (
                        <Button
                          ...
                          variant="ghost"
                          aria-label={
                            locale === zhCN ? "选择语言" : "Select language"
                          }
                          aria-controls={menuId}
                          aria-expanded={open}
                          aria-haspopup="menu"
                        ...
                        <div className="accent-options">
                          {(
                            ...
                            return (
                              ...
                              >
                                <span
                                  className={`accent-swatch accent-swatch-${accentValue ?? themeValue}`}
                                />
                                <span>{label}</span>
                                {selected ? (
                                  <Check aria-hidden="true" size={15} />
                    ...
                    >
                      {({ close }) => (
                        <div
                          className="-my-1"
                          data-testid="playground-notifications"
                        >
                          <div className="flex items-center justify-between border-b border-[var(--lumen-color-surface-muted)] px-4 py-3">
                            <span className="text-[15px] font-medium text-[var(--lumen-color-text-strong)]">
                              通知
                            </span>
                            <Badge size="sm" variant="danger">
                              2
                            ...
                            {[
                              ...
                              >
                                ...
                                <span className="min-w-0 flex-1">
                                  <span className="block truncate text-[14px] font-normal leading-5 text-[var(--lumen-color-text)]">
                                    {title}
                                  </span>
                                  <span className="mt-1 block text-[12px] leading-5 text-[var(--lumen-color-text-placeholder)]">
                                    {time}
                                  </span>
                                </span>
                              </button>
              ...
              <div className="main-content">
                <ActiveDemoContext.Provider
                  value={{
                    ...
                    copiedCodeTitle,
                    onToggleCode: (title) =>
                      setExpandedCodeTitle((current) =>
                        current === title ? undefined : title,
                      ),
                    onCopyCode: (title, code) =>
                      void copyActiveDemoCode(
                        title,
                                ...
                                <div className="flex flex-col gap-3">
                                  ...
                                  >
                                    <div className="select-none touch-pan-y rounded-lg bg-[var(--lumen-color-surface-muted)] p-5 text-center text-[14px] text-[var(--lumen-color-text)]">
                                      PC
                                      端右键，移动端长按；菜单从触发位置展开，靠近窗口边缘时自动翻转
                                    </div>
                                  </ContextMenu>
                                  <Typography variant="caption" color="muted">
                              ...
                              <DemoCard title="Avatar">
                                <div className="avatar-row">
                                  ...
                                  <Avatar
                                    ...
                                    style={{
                                      backgroundColor:
                                        "var(--lumen-color-success-soft)",
                                      borderColor:
                                        "var(--lumen-color-success-border)",
                                  ...
                                  <Avatar
                                    ...
                                    style={{
                                      backgroundColor:
                                        "var(--lumen-color-warning-soft)",
                                      borderColor:
                                        "var(--lumen-color-warning-border)",
                                  ...
                                  <Avatar
                                    ...
                                    style={{
                                      backgroundColor:
                                        "var(--lumen-color-danger-soft)",
                                      borderColor:
                                        "var(--lumen-color-danger-border)",
                                    ...
                                    {({ isSubmitting, submitError }) => (
                                      <>
                                        <fieldset
                                          ...
                                          className="grid min-w-0 gap-x-4 gap-y-0 border-0 p-0 pad:grid-cols-2"
                                        >
                                          {(
                                            [
                                              {
                                        ...
                                        {formSubmitted && (
                                          <Alert
                                            ...
                                            title={
                                              language === "en-US"
                                                ? "All four fields passed. Demo submission completed."
                                                : "四个字段全部校验通过，模拟提交成功。"
                                            }
                                          >
                                            {formSubmitted}
                                  ...
                                  items={[
                                    {
                                      ...
                                      content: (
                                        <div className="flex h-full flex-col justify-end bg-gradient-to-br from-indigo-600 to-violet-500 p-7 pb-12 text-white pad:px-16 l:px-16">
                                          <strong className="text-[22px] font-semibold">
                                            移动体验升级
                                          </strong>
                                          ...
                                          <div>
                                            ...
                                            <div className="mt-6 grid grid-cols-1 gap-4 l:grid-cols-3">
                                              {[
                                                ...
                                                <div
                                                  key={metric.label}
                                                  className="rounded-[8px] border border-[var(--lumen-color-border)] p-4"
                                                >
                                                  <Typography
                                                    variant="body"
                                                    color="muted"
                          ...
                          return (
                            <GallerySection key={section.id} section={section}>
                              ...
                              <DemoCard title="List" wide>
                                <List aria-label="重点事件">
                                  <ListItem
                                    ...
                                    leading={
                                      <AlertTriangle
                                        className="text-[var(--lumen-color-danger)]"
                                        size={18}
                                      />
                                    }
                                    meta={
                                  ...
                                  <ListItem
                                    ...
                                    leading={
                                      <MapPin
                                        className="text-[var(--lumen-color-warning)]"
                                        size={18}
                                      />
                                    }
                                    meta="8 分钟前"
                                  ...
                                  <ListItem
                                    ...
                                    leading={
                                      <Check
                                        className="text-[var(--lumen-color-success)]"
                                        size={18}
                                      />
                                    }
                                    meta={
                                    ...
                                    <div className="divide-y divide-[var(--lumen-color-surface-muted)] px-4">
                                      {Array.from(
                                        ...
                                        (_, index) => (
                                          <div
                                            key={index}
                                            className="py-3 text-[13px] text-[var(--lumen-color-text-secondary)]"
                                          >
                                            K{18 + index} 路段监测记录 ·{" "}
                              ...
                              <DemoCard title="Divider" wide>
                                <div>
                                  <p className="text-[13px] text-[var(--lumen-color-text-secondary)]">
                                    G65 K18+900 南向路段
                                  </p>
                                  <Divider />
                                  <p className="text-[13px] text-[var(--lumen-color-text-secondary)]">
                                      ...
                                      <div>
                                        <div className="flex items-start justify-between gap-3">
                                          <div>
                                            ...
                                            <div className="mt-1 text-[12px] text-[var(--lumen-color-text-muted)]">
                                              G65 K18+900 南向
                                            </div>
                                          </div>
                                        ...
                                        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-[13px]">
                                          <span className="text-[var(--lumen-color-text-muted)]">
                                            最后同步
                                          </span>
                                          <span className="text-right text-[var(--lumen-color-text)]">
                                            2 分钟前
                                          </span>
                                          <span className="text-[var(--lumen-color-text-muted)]">
                                            今日事件
                              ...
                              <div className="w-full max-w-2xl">
                                <FileList
                                  items={[
                                    {
                                      id: "pdf",
                                      name: "应用于低压配电物联网的断路器管理方法及系统_20231024032440.pdf",
                                      size: 1258291,
                                      badge: {
                                        label: "已上传",
                                        variant: "success",
                                  ...
                                  renderActions={(file) => (
                                    <Button
                                      ...
                                      title="查看"
                                      onClick={() =>
                                        Toast.info(`查看附件：${file.name}`)
                                      }
                                    />
                                  )}
                                  onRemove={(file) =>
                                    Toast.info(`移除附件：${file.name}`)
                                  }
                                />
                              </div>
                            </DemoCard>
                              ...
                              <div className="w-full max-w-2xl">
                                <FileList
                                  ...
                                  items={[
                                    {
                                      id: "doc",
                                      name: "这是一份完整显示名称并支持自动换行的项目设计说明文档.docx",
                                      size: 8192,
                                      badge: {
                                        label: "已归档至项目文档资料库",
                                        variant: "neutral",
            ...
            <div>
              <FormField label="负责人">
                <Select
                  ...
                  onChange={(value) =>
                    setDialogSelectValue(value as string | null)
                  }
                  placeholder="选择负责人"
                />
              </FormField>
