...
const items: FileListFile[] = [
  {
    ...
    name: "report.pdf",
    size: 1258291,
    badge: { label: "Awaiting a very long review", variant: "warning" },
  },
...
describe("FileList", () => {
  it("defaults to compact badges and forwards updated width settings to each item", () => {
    ...
    const widths = () =>
      screen
        ...
        .map((item) =>
          item.style.getPropertyValue("--lumen-file-badge-max-width"),
        );
  ...
  it("supports badge width on standalone items while preserving caller styles", () => {
    ...
    expect(item.style.getPropertyValue("--lumen-file-badge-max-width")).toBe(
      "20%",
    );
  ...
  it("renders remote metadata with file type icons and optional sizes", () => {
    ...
    expect(
      container.querySelector('[data-file-type-category="pdf"]'),
    ).toBeInTheDocument();
    expect(
      container.querySelector('[data-file-type-category="spreadsheet"]'),
    ).toBeInTheDocument();
  ...
  it("renders clamped progress below each matching file name", () => {
    ...
    expect(progressBars[0]?.firstElementChild?.firstElementChild).toHaveStyle({
      width: "42%",
    });
    expect(progressBars[0]).toBe(
      screen
  ...
  it("places compact size before the badge while keeping progress below the name", () => {
    ...
    expect(
      size.compareDocumentPosition(badge) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it.each(["report.pdf", "Awaiting a very long review"])(
    "only shows a tooltip for overflowing text: %s",
    (text) => {
      vi.useFakeTimers();
      let width = 500;
  ...
  it("renders trailing actions with the matching file independently of removal", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Preview budget.xlsx" }),
    );
  ...
  it("collapses custom actions into a mobile overflow menu", () => {
    ...
    render(
      <FileList
        ...
        onRemove={remove}
        renderActions={(file) => (
          <button title="Preview" onClick={() => preview(file.id)}>
            Preview {file.name}
          </button>
        )}
      />,
...
describe("FileUpload file list integration", () => {
  it("renders per-file upload progress inside each file row", () => {
    ...
    expect(screen.getByText("5 B").parentElement).toBe(
      screen.getByText("report.pdf").closest("li"),
    );
  ...
  it("forwards display options and keeps removal tied to the original File", () => {
    ...
    expect(
      screen.getByRole("button", { name: "移除 report.pdf" }),
    ).toBeDisabled();
    rerender(
      <FileUpload value={files} onChange={onChange} showFileList={false} />,
    );
