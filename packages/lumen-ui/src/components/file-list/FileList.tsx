        ...
        React.Children.map(actions, (action) => {
                ...
                return (
                        <DropdownMenuItem
                                key={action.key}
                                disabled={
                                        parentDisabled || actionProps.disabled
                                }
                                onClick={(event) => {
...
export function FileListItem({
        ...
        const normalizedProgress =
                file.progress === undefined || !Number.isFinite(file.progress)
                        ? undefined
        ...
        const renderRemoveButton = () =>
                onRemove ? (
                        <button
                                type="button"
                                disabled={disabled}
                                aria-label={locale.fileUpload.removeFile(
                                        file.name,
                                )}
        ...
        return (
                <li
                        ...
                        style={
                                {
                                        ...style,
                                        "--lumen-file-badge-max-width":
                                                typeof badgeMaxWidth ===
                                                "number"
                                                        ? `${badgeMaxWidth}px`
                                                        : badgeMaxWidth,
                ...
                >
                        ...
                        <span className="min-w-0 flex-1 text-left">
                                <OverflowText
                                        text={file.name}
                                        wrap={wrapName}
                                        className="text-[14px] font-normal text-[var(--lumen-color-text)]"
                                />
                        ...
                        {file.badge ? (
                                <Badge
                                        ...
                                        size="md"
                                        className="min-w-0 max-w-[min(var(--lumen-file-badge-max-width),35%)] shrink-0"
                                >
                                        ...
                                        {actions == null ? null : (
                                                <DropdownMenu
                                                        ...
                                                        trigger={({
                                                                ...
                                                                <button
                                                                        type="button"
                                                                        aria-label={locale.fileUpload.fileActions(
                                                                                file.name,
                                                                        )}
                                                                        aria-controls={
                                                                                open
...
export function FileList({
        ...
        return (
                <ul
                        {...props}
                        className={cn(
                                "m-0 min-w-0 list-none space-y-2 p-0",
                                className,
                        )}
                >
                        {items.map((file) => (
